...
yI2 = filter(b,a,yI);           % Filtrera xI2
yQ2 = filter(b,a,yQ);           % Filtrera xQ2

yI3= filter(b,a,[yI2;zeros(N/2,1)]);
yI3= yI3(N/2+1:max(size(yI2))); % Remove first N/2 points since filter adds delay
yQ3= filter(b,a,[yQ2;zeros(N/2,1)]);
yQ3= yQ3(N/2+1:max(size(yQ2)));
