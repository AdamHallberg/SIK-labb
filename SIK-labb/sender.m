...
x = [known_chirp; x; known_chirp];
%%  Bandpass filter to stay in desired bandwidth
% filter signal
%[b,a]=fir1(N,[140, 160]/(400));               % Designa filter

%x  = filter(b,a,[x;zeros(N/2,1)]);
%x  = 400*x(N/2+1:max(size(x))); % Remove first N/2 points since filter adds delay
